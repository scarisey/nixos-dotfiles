  ...
  config = lib.mkIf cfg.enable {
    systemd.services.llama-cpp = {
      ...
      serviceConfig = {
        ...
        DevicePolicy = "closed"; # explicit allowlist, deny everything else
        DeviceAllow = [
          "/dev/kfd rw" # AMD compute (KFD)
          "/dev/dri rw" # render nodes (renderD128, card0, …)
        ];
        SupplementaryGroups = ["render" "video"]; # DynamicUser needs group membership for device access
        ...
        PrivateMounts = true;
        PrivateTmp = true;
        # PrivateUsers = true;             # REMOVED: user namespace breaks render/video group resolution
        ProtectClock = true;
        ...
        MemoryDenyWriteExecute = false; # ROCm JIT-compiles GPU kernels at runtime — must be false
        ...
        SystemCallFilter = [
          "@system-service"
          "~@privileged"
          "personality" # re-allow: ROCm probes it; was removed by ~@privileged above
        ];
